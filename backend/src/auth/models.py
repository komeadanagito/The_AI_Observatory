...
class User(Base):
    ...
    birth_date_encrypted: Mapped[Optional[bytes]] = mapped_column(
        LargeBinary, nullable=True, comment="出生日期（AES-256-GCM 加密）"
    )
    birth_time_encrypted: Mapped[Optional[bytes]] = mapped_column(
        LargeBinary, nullable=True, comment="出生时间（AES-256-GCM 加密）"
    )
    birth_place_encrypted: Mapped[Optional[bytes]] = mapped_column(
        LargeBinary, nullable=True, comment="出生地点（AES-256-GCM 加密）"
    )
    ...
    language: Mapped[str] = mapped_column(
        String(5), default="zh", nullable=False, comment="界面语言: zh/en"
    )
    ...
    mbti_sessions: Mapped[List["MBTISession"]] = relationship(
        "MBTISession",
        back_populates="user",
        cascade="all, delete-orphan",
        lazy="selectin",
