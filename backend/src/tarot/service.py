...
class TarotService:
    ...
    _session_interpret_cache: dict[str, tuple[str, datetime]] = {}
    ...
    async def draw_cards(
        ...
        if selected_card_ids:
            ...
            if len(selected_card_ids) != card_count:
                raise ValueError(
                    f"选择的牌数量({len(selected_card_ids)})与牌阵要求({card_count})不符"
                )
    ...
    def _get_question_hash(
        self, spread_type: str, cards: list, question: Optional[str]
    ) -> str:
        """生成问题哈希，用于缓存"""
    ...
    def cache_session_interpretation(
        self, session_id: str, interpretation: str
    ) -> None:
        """按 session_id 缓存最终解读，供前端轮询兜底。"""
        self._session_interpret_cache[session_id] = (interpretation, datetime.utcnow())

    def get_session_interpretation(self, session_id: str) -> Optional[str]:
        cached = self._session_interpret_cache.get(session_id)
        if not cached:
            return None

        interpretation, timestamp = cached
        if datetime.utcnow() - timestamp < self.DRAW_CACHE_TTL:
            return interpretation

        del self._session_interpret_cache[session_id]
        return None
...
- 不要使用 markdown 格式，直接输出文字"""
        ...
        for card in cards:
            position_info = next(
                (p for p in positions if p["index"] == card["position"]), None
            )
            position_name = (
                position_info["name_zh"] if position_info else f"位置{card['position']}"
            )

            card_info = card["card"]
            orientation = "逆位" if card["is_reversed"] else "正位"
            meaning = (
                card_info["meaning_reversed"]
                if card["is_reversed"]
                else card_info["meaning_upright"]
            )
